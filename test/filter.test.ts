        ...
        switch (support.sequelize.getDialect()) {
          case "sqlite":
            expect(sql).to.contain(
              "((`table`.`column1` LIKE '%test%') AND (`table`.`column2` = 'abc' OR CAST(`column3` AS DATE) = '2017-11-01T00:00:00'))"
            );
            break;
          case "mysql":
            expect(sql).to.contain(
          ...
          case "postgres":
            expect(sql).to.contain(
              '(("table"."column1" ILIKE \'%test%\') AND ("table"."column2" = \'abc\' OR CAST("column3" AS DATE) = \'2017-11-01T00:00:00\'))'
            );
            break;
          case "mssql":
            expect(sql).to.contain(
              "(([table].[column1] LIKE N'%test%') AND ([table].[column2] = N'abc' OR CAST([column3] AS DATE) = N'2017-11-01T00:00:00'))"
            );
            break;
        }
      });
